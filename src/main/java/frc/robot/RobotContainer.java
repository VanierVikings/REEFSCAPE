...
public class RobotContainer {
  ...
  public RobotContainer() {
    ...
    NamedCommands.registerCommand("scoreL2", m_elevator.setPivot(Setpoint.kLevel2).andThen(Commands.waitSeconds(0.2)).andThen(m_elevator.setElevator(Setpoint.kLevel2)).andThen(Commands.waitSeconds(0.4)).andThen(m_endEffector.setPosition(SetpointEE.kPlaceL2)).andThen(Commands.waitSeconds(0.35)).andThen(m_endEffector.spin(0.7).withTimeout(1)));
    ...
    NamedCommands.registerCommand("Descore High", m_elevator.setPivot(Setpoint.KAlgaeHighStart).andThen(Commands.waitSeconds(0.4)).andThen(m_elevator.setElevator(Setpoint.KAlgaeHighStart)).andThen(Commands.waitSeconds(0.2)).andThen(m_endEffector.setPosition(SetpointEE.kRest).andThen(Commands.waitSeconds(0.15)).andThen(m_elevator.setPivot(Setpoint.KAlgaeHighEnd).andThen(Commands.waitSeconds(0.1)).andThen(m_endEffector.setPosition(SetpointEE.kRest)))));
    NamedCommands.registerCommand("LEFTautoAlign", drivetrain.defer(() -> drivetrain.autoAlign(drivetrain.getBranchPose(branchSide.leftBranch))));
    NamedCommands.registerCommand("RIGHTautoAlign", drivetrain.defer(() -> drivetrain.autoAlign(drivetrain.getBranchPose(branchSide.rightBranch))));
    // NamedCommands.registerCommand("pulseL1", );
